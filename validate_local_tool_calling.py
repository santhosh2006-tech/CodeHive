...
# 1. Initialize client pointing to Ollama's local OpenAI-compatible endpoint
# Ollama runs on port 11434 by default.
client = OpenAI(
    base_url="http://localhost:11434/v1",
    api_key="ollama"  # Ollama does not require a real key, but a placeholder is needed for the OpenAI SDK
)

# Recommend a 7B/8B class model optimized for coding and tool call support
MODEL_NAME = "qwen2.5-coder:7b"

# Define the tools exactly as structured in CodeHive's tools.py schema
tools_schema = [
    {
        "type": "function",
        "function": {
...
def run_test_case_1():
    print("\n" + "="*50)
    print("TEST CASE 1: Single Tool Call Validation")
    print("Target: Have the model write a file named 'hello_local.txt'.")
    print("="*50)
    ...
    messages = [
        ...
        {"role": "user", "content": "Write a file named 'hello_local.txt' containing 'Hello from local Ollama!'."}
    ]
...
def run_test_case_2():
    print("\n" + "="*50)
    print("TEST CASE 2: Chained Sequential Tool Calls Validation")
    print("Target: Read 'hello_local.txt', then write 'hello_local_modified.txt'.")
    print("="*50)
    
    # Step A: Simulate reading the file first
    messages = [
        {"role": "system", "content": "You are a helpful coding assistant. Fulfill the user request using sequential tool calls step-by-step."},
        {"role": "user", "content": "Read the file 'hello_local.txt' first, then write a modified file 'hello_local_modified.txt' adding 'Modified locally!' to the end."}
    ]
    ...
    try:
        ...
        # Append assistant's call and the simulated tool response to the history
        ...
        # Step B: Request subsequent modification call
...
if __name__ == "__main__":
    print("Ollama Local Tool-Calling Validator")
